...
{
	if (line == 0)
		fprintf(mem, "ERROR(LINKER): %s\n", msg);
	else
		fprintf(mem, "ERROR(%d): %s\n", line, msg);
	numErrors++;
}

void warning(const char* msg, int line)
{
	fprintf(mem, "WARNING(%d): %s\n", line, msg);
	numWarnings++;
}
...
//					printf("Made a new scope \"%s\" \n", str.c_str());
			...
			case ExpK:
					...
					if (tree->attr.op != DEC && tree->attr.op != INC)
					{
						...
						//if (leftType != UndefinedType && rightType != UndefinedType) /////////////////////UndeclaredType
						{
							if (returnVar != NULL)
							{
								...
								tree->isArray = leftArray && rightArray;
							}
						}
					}
					else
				...
				{
					if (tree->expType == Char && tree->isArray)
					{
						tree->memory = Global;

						tree->location = goffset - 1;
						goffset -= tree->size;
					}
					else
					{
				...
				else if (tree->subkind.exp == IdK)
				{
					...
					else if (!returnVar->isInitialized && !returnVar->isGlobal && !returnVar->ignoreWarn && !tree->ignore)
					{
						...
						tree->memory = returnVar->memory;
						tree->location = returnVar->location;
						tree->size = returnVar->size;

						tree->isArray = returnVar->isArray;
						tree->isStatic = returnVar->isStatic;
						tree->isGlobal = returnVar->isGlobal;
					}
					else
					{
						tree->memory = returnVar->memory;
			...
			case DeclK:
				if (tree->subkind.decl == VarK)
				{
					...
					if (compoundNum > 0)
					{
						...
						else
						{
							if (tree->isStatic)
							{
								tree->location = goffset;
								goffset -= tree->size;
							}
							else
							{
								tree->location = foffset;
								foffset -= tree->size;
							}
						}
					}
				...
				if (tree->subkind.decl == FuncK)
				{
					...
					tree->size = -2;
					foffset = -2; //If it has return type (no matter)
					offset.push(foffset);
...
//						printf("Declared %s as a new variable of %s ", tree->attr.name, expType);
		...
		{
			if (tree->subkind.stmt == CompoundK || tree->subkind.stmt == ForK)
			{
				tree->size = foffset;
